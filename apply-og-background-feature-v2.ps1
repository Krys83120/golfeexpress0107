...
# Verif anti-mojibake : si ce script est lu avec le mauvais encodage (BOM
# manquant/ignore), les caracteres accentues ecrits dans les fichiers seront
# corrompus (ex: "e9" devient "Ã©"). On verifie ici que ce n'est PAS le cas.
$mojibake = Select-String -Path "apps/admin/src/pages/SeoPage.tsx" -Pattern 'Ã.' -SimpleMatch:$false -ErrorAction SilentlyContinue
if ($mojibake) {
    Write-Host ""
    Write-Host "ATTENTION : caracteres corrompus detectes dans SeoPage.tsx !" -ForegroundColor Red
    Write-Host "Ta version de PowerShell n'a pas lu ce script en UTF-8 malgre le BOM." -ForegroundColor Red
    Write-Host "Ne commit PAS. Dis-le a Claude, on trouvera une autre methode (ex: pwsh 7)." -ForegroundColor Red
} else {
    Write-Host ""
    Write-Host "Verif encodage OK : pas de caracteres corrompus detectes." -ForegroundColor Green
}
